    ...
    <ul class="space-y-3">
        ...
        <li>
            <details {{ request()->routeIs('data_compare.transaction_detail.*') ? "open" : ""}}>
                <summary class="hover:bg-yellow-400 p-1 pr-2 flex items-center justify-between text-base font-medium rounded-lg {{ request()->routeIs('data_compare.*') ? "bg-yellow-400" : "" }}">
        ...
        <li>
            <details {{ request()->routeIs('digital_receipt.sync_data.*') ? "open" : ""}}>
                ...
                <ul id="submenu" class="ml-5 border-l-[1.5px] border-slate-200 mt-3 space-y-2">
                    <li>
                        <a href="{{ route("data_compare.digital_receipt.dashboard") }}" class="text-base font-normal pl-5 border-l-[1.5px] -ml-[1.5px] text-gray-400 hover:text-blue-950 hover:border-blue-950{{ request()->routeIs('data_compare.digital_receipt.*') ? "text-blue-950 border-blue-950" : "text-gray-400" }}">{{ __("Data Compare") }}</a>
                    </li>
                </ul>
            </details>
        </li>
